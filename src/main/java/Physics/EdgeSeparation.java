...
public class EdgeSeparation {
    public Vector2D findSmallestSeparations(Rectangle r1, Rectangle r2) {
        ...
        float rightToLeft = Math.abs(r1.x + r1.width - r2.x);
        results.x = Float.compare(Math.min(results.x, rightToLeft), rightToLeft) == 0
                ? -rightToLeft
                : results.x;
        ...
        float bottomToTop = Math.abs(r1.y + r1.height - r2.y);
        results.y = Float.compare(Math.min(results.y, bottomToTop), bottomToTop) == 0
                ? -bottomToTop
                : results.y;
